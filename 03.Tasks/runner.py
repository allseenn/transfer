...
from yell.spiders.recomend import RecomendSpider
import os
...
if __name__ == "__main__":
    ...
    settings.set('FEED_EXPORT_FIELDS', [f'header', 'text', 'verdict', 'link'])
    settings.set('FEEDS', {
                            query+'.csv': {
                                'format': 'csv',
                                'item_export_kwargs': {
    ...
    settings.set('ROTATING_PROXY_LIST_PATH', '../../proxy_list.txt')
   ...
   'rotating_proxies.middlewares.BanDetectionMiddleware': 110,
    ...
    process.crawl(RecomendSpider, query=query)
    process.start()